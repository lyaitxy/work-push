...
import com.example.workpush.entity.resp.AliJobResp;
...
public class AlibabaWorkServiceImpl implements AlibabaWorkService {
    ...
    public void pushwork() {
        ...
        m.put("batchId", "");
        m.put("categoryType", "internship"); //internship 暑期实习， project 项目制日常实习
        ...
        List<AliJob> list = new ArrayList<>();
        ...
        List<AliJobResp> res = new ArrayList<>();
        // 列表不为空
        if(!list.isEmpty()) {
            // 判断今天是否已发
            list.forEach(item -> {
                ...
                if(!isPush) {
                    ...
                    res.add(BeanUtil.copyProperties(item, AliJobResp.class));
                }
            });
        }
        if(!res.isEmpty()) {
